...
class MacOSInstallHelper:
    ...
    def get_installation_status(self) -> dict:
        ...
        press_path = self.platform_manager.get_press_path()
        return {
            'press_path': str(press_path),
            'bin_dir': str(self.bin_dir),
            'config_dir': str(self.config_dir),
            'is_press_installed': self._verify_hvym_press_installation() if press_path.exists() else False
        }
    ...
    def check_macos_permissions(self) -> list:
        ...
        try:
            ...
            # If hvym_press exists, ensure it's executable
            try:
                press_path = self.platform_manager.get_press_path()
                if press_path.exists() and not os.access(press_path, os.X_OK):
                    issues.append(f"Binary not executable: {press_path}")
            except Exception:
                issues.append("Could not validate hvym_press binary permissions")
        except Exception:
...
def main():
    ...
    print(f"Expected hvym_press path: {helper.platform_manager.get_press_path()}")

    # Check if hvym_press is supported
    if not helper.platform_manager.is_hvym_press_supported():
        print("hvym_press is not supported on this architecture (Apple Silicon)")
        return

    # Check current status
    status = helper.get_installation_status()
    if status.get('is_press_installed'):
        print("hvym_press is already installed and working")
    else:
        print("hvym_press is not installed or not working")

        # Try to install
        print("\nAttempting installation...")
        result = helper.install_hvym_press_cli()
